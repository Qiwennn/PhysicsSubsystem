...
// Uses the implicit form: ax + by + cz + d = 0 (not ax + by + cz = d)
struct Plane
{
	float a, b, c, d;
	...
	float getD() { return d; }
	Vector3 getN() { return Vector3(a, b, c); }                // inward normal (points inside the box)
	Vector3 getOutsideN() { return getN() * -1.0f; }
	float getFloorHeight() { return (-d) / b; }

	void normalize(const Vector3 &p)
	{
		d = (a * p.getX() + b * p.getY() + c * p.getZ()) * -1;
	}

	bool checkPlaneIntersect(Plane &target)
	{
		return !(getN().crossProduct(target.getN()) == Vector3());
	}
	...
	{
		float s = a * p.getX() + b * p.getY() + c * p.getZ() + d;
		return (s >= 0.0f);
	}
