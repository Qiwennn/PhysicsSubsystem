...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	// physics component for soldier
	{
		...
		pPhys->addDefaultComponents();
		pPhys->attachToSceneNode(hSN);
		pPhys->setStatic(false);
		pPhys->setMass(1.0f);
		pPhys->addSphere(0.6f);
		addComponent(hPhys);
		if (PhysicsManager::InstanceHandle().isValid())
			PhysicsManager::Instance()->registerComponent(hPhys);
	}
