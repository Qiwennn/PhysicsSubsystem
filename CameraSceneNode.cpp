...
CameraSceneNode::CameraSceneNode(PE::GameContext &context, PE::MemoryArena arena, Handle hMyself)
...
void CameraSceneNode::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
	...
	Matrix4x4 &worldRef = m_worldTransform;

	Vector3 pos = worldRef.getPos();
	Vector3 n   = worldRef.getN();
	Vector3 up  = worldRef.getV();
	Vector3 target = pos + n;

	m_worldToViewTransform = CameraOps::CreateViewMatrix(pos, target, up);

	m_worldTransform2 = worldRef;
	m_worldTransform2.moveForward(Z_ONLY_CAM_BIAS);

	Vector3 pos2 = m_worldTransform2.getPos();
	Vector3 n2   = m_worldTransform2.getN();
	Vector3 up2  = m_worldTransform2.getV();
	Vector3 target2 = pos2 + n2;

	m_worldToViewTransform2 = CameraOps::CreateViewMatrix(pos2, target2, up2);

	PrimitiveTypes::Float32 aspect =
		(PrimitiveTypes::Float32)(m_pContext->getGPUScreen()->getWidth()) /
		(PrimitiveTypes::Float32)(m_pContext->getGPUScreen()->getHeight());
	...
	m_viewToProjectedTransform = CameraOps::CreateProjectionMatrix(
		verticalFov, aspect, m_near, m_far);
